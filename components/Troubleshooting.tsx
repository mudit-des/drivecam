...
const FAQ_GROUPS: FaqGroup[] = [
  {
    id: "connecting",
    category: "Connecting Your DriveCam",
    items: [
      {
...
const FILTERS: { id: string; label: string }[] = [
  { id: "all", label: "All" },
  ...FAQ_GROUPS.map((g) => ({ id: g.id, label: g.category })),
];
...
function FaqAccordionItem({ item }: { item: FaqItem }) {
  ...
  return (
    <div>
      ...
      {open && (
        <div className="pb-4 flex flex-col gap-3">{item.answer}</div>
      )}
    </div>
  );
}
...
export function Troubleshooting() {
  ...
  const visibleGroups = useMemo(
    () =>
      activeFilter === "all"
        ? FAQ_GROUPS
        : FAQ_GROUPS.filter((g) => g.id === activeFilter),
    [activeFilter]
  );
